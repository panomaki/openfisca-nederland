...
These elements are described in different folders. All the modelling happens within the `openfisca_nederland` folder.
...
```sh
pip install openfisca_nederland
```
...
```sh
git clone https://github.com/openfisca/openfisca-nederland.git
cd openfisca-nederland
...
Substitute your package's country name for `openfisca_nederland` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_nederland/situation_examples/couple.json \
